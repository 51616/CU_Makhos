...
def build_unique_examples(examplesHistory):
    ...
    for examples in examplesHistory:
        ...
        for move in examples:
            position, policy_vector, reward = move[0], move[1], move[2]
            turn, stale, valids = move[3], move[4], move[5]
            #s = strRepr(position)
            # print(s)
            ...
            s = position.tobytes() + bytes([turn]) + bytes([stale])
    ...
    for s in stateMap.keys():
        ...
        if size > 1:
            ...
            for (position, pi, v, turn, stale, valids) in predictions:
                sum_v += v
                if len(sum_pi) == 0:
            ...
            assert len(sum_pi) == len(predictions[0][1]), str(
                len(sum_pi))+"!="+str(len(predictions[0][1]))

            position = predictions[0][0]
            newExamples.append((position, sum_pi, sum_v, turn, stale, valids))
        else:
